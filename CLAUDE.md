...
- **`swviz/models/dataport.py` and `flow_control_port.py` carry no comments at all.** They
  are the transport-placement reference model published in the SWI3S specification, and a
  test enforces it. Put the explanation in the commit message instead.

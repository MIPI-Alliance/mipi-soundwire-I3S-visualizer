...
See [PERFORMANCE.md](PERFORMANCE.md) for the full loop — the recurring
...
[spacing_bug.md](spacing_bug.md), and the partial-channel-group rationale in
...
3. **Check what this repository will run** — its Actions state and required checks may differ
   ...
   another.
4. Push the release branch's **content tip** — the last real commit of the release, not a
   merge commit — and never force-push `main`.

Keep the published tree **identical to what the release tag attests to** and to what the
two-platform gate was run against. Fix anything you find in the NEXT cycle rather than
patching the published copy: a tree that differs from the tag claiming to describe it makes
both untrustworthy.

## Deferred work

Findings consciously deferred from a review cycle live in [TECH_DEBT.md](TECH_DEBT.md) so

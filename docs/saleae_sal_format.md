...
16      ...     fixed/metadata fields (a byte that is 1 here is NOT the initial state)
...
...     variable-length channel-name records (ASCII, e.g. "me1")
...     transition-block chain (see 3.4) — the initial state lives here
```

The variable-length metadata is exactly why a fixed block offset (the old `61`) is
wrong: the block chain floats per file/channel, so the reader **scans** for it.
...
3. The app now imports `.sal` directly (**File ▸ Open Capture (.sal / .csv / .bin)**);

...
matched configuration: four agree to four decimals, the Manager keeper leg agrees at *both*
release placements, the peripheral-launched ones differ by exactly this anchor conversion,
P→P **hold** differs by the anchor alone, and P→P
...
**A CROSS-CHECK IS ONLY AS INDEPENDENT AS ITS INPUTS, and this file learned that the
expensive way on 2026-08-18.** The matched configuration set `Man_t_DZ_max_ns` from
`E.MAN_ANALOG_MAX` — the reference's *launch* range — because the reference used the same
constant for its release maximum. `Man_t_DZ` is its own tabulated parameter at 0–10 ns. So the
test asserted agreement to four decimal places on a number neither model should have produced,
and it did that for as long as both models agreed to be wrong. **Two copies of one error are
indistinguishable from a cross-check passing.** The rule that follows: a matched configuration
must name the *specification's* parameter, not read the value out of the artefact under test.
Fixed on both sides; the reference's no-mechanism M→P baseline moved −12.90 → +0.10 ns.
...
**And one scope error the cross-check could not have caught, for the same reason.** Both
artefacts excluded `PP_hold_ho` (the reference's `hold_pp`) from what bounds the bus, on the
grounds that peripheral-to-peripheral data is not guaranteed. The inequality names only the
device turning on out of high-Z and the device whose hold window that ends — the owner of the
level at risk does not appear in it — so when the **Manager** released, the level is
Manager→peripheral data and a peripheral sink must hold it. −7.67 ns at zero handover UIs
here, −3.60 at the reference's fixed corner. Rod found it from the bus side rather than from
either model. `tests/test_timing_cross_spec.py::test_the_out_of_scope_split_matches_the_legs_own_terms`
now states the rule and checks it against the terms; `PP_hold` keeps the exclusion, because for
that one the claim is true.

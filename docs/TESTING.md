...
> 29 Python suites + the native C++ suite.
...
### 3.2 Python suites
Location: `swi3s-studio/tests/`, 29 files, one concern per file. Each is a plain script
...
The quickest path is the bundled runner, which runs every Python suite, judges
each by exit code, and prints a pass/fail summary:

```bash
cd swi3s-studio
bash tests/run_all.sh                  # all Python suites
...
- **No CI yet.** `tests/run_all.sh` runs both layers locally and exits non-zero on any
  failure, but there is no GitHub Actions job and no enforced pre-commit run. *Risk: a
  suite can rot between manual runs.* (Cheapest next step: a CI job that builds the core
  and runs `tests/run_all.sh --build --native`.)
- **Performance is untested.** architecture.md calls for throughput / pan-zoom latency /
  memory-ceiling benchmarks on a synthetic multi-GB capture; none exist. Large-capture
  behaviour is currently validated only manually on the real Lusk files.

...
bool C8b10bDecoder::PushBit(bool bit, Symbol& out)
{
    ...
    // A K.28.7 comma (re)establishes symbol alignment at any bit boundary — UNLESS
    // framing is locked because we are mid-phase inside a known-length data region.
    // There, an SPM-shaped bit pattern in payload/CRC bytes is NOT a phase boundary
    // (SWI3S §7.2.2 {ASW1907}: hold alignment by counting bits mod 10; §8.2.1
    // {ASW2206}: a mid-phase SPM only ARMS header detection, it does not re-frame
    // the in-progress phase), so we fall through to the ordinary mod-10 path.
    bool isComma = !mFramingLocked &&
                   ((mWindow == swi3s::kCommaSymbolRDm1) ||

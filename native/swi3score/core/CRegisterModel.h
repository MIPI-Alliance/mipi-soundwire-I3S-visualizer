...
public:
    ...
    // What-if override: force ONLY the rank the address names — a _NEXT-base address
    // stages _NEXT, a _CURR alias (or a single-ranked register) sets _CURR. It never
    // touches the other rank and never commits, so editing _NEXT doesn't copy to _CURR
    // (nor promote any other staged register). BuildConfig reads _CURR, so only a _CURR
    // override changes the decoded config.
    void ForceOverride(int dev, U32 addr, U8 value);
    ...
    // Committed (_CURR) Interval for data port `dp` on device `dev` — rows per
    // interval minus 1 (0 == every row transports, i.e. "Interval == 1 Row" in the
    // spec's row-count sense). Reads OffIntLo_N/IntHi_N via the same committed-state
    // get() as decodeDp, so it reflects whatever the port's Interval last committed
    // to, regardless of what the current command is writing. Unseen -> reset (0).
    int CurrentInterval(int dev, int dp) const;
    ...
    // True if a WriteA32 to `addr` takes effect on the COMMITTED (_CURR) state IMMEDIATELY
    // — i.e. a single-ranked register, applied by applyWrite the moment it's written, with
    // no Commit. (Dual-ranked writes stage in _NEXT and wait for a Commit.) The streaming
    // decode uses this to reconfigure the payload engine right after such a write, so the
    // audio/samples decode reflects the register state however it was set — not only at a
    // commit. See applyWrite / isDualRanked.
    bool WriteIsImmediate(U32 addr) const;

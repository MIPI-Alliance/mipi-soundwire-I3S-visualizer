...
private:
    ...
    void maybeConfigureFromSnoop(int columnCount, int sectionIndex);
    ...
    // When no command completes for a long run of UIs, the framing is stale, so
    // we re-detect the column count on a fresh window and re-hunt the comma.
    long   mUisSinceCommand = 0;

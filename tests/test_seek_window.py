...
Run: python3 -m pytest tests/test_seek_window.py
"""
import swi3score
...
def test_seek_window_matches_full_from_comma():
    # Frame within the demo's final 16-column region (it starts in safe-lock-2 and widens
    # 2 -> 8 -> 16), decoding from that segment's Column-0 origin on the continuous row
    # scale — the same way the symbol viewer windows into a multi-config capture.
    sess = Session.from_demo(300)
    seg = sess.segments[-1]
    cols = seg["column_count"]
    origin = seg["start_ui"]
    base = seg["row_base"]
    full = swi3score.decode_symbols(sess.capture.sample_source(), cols, 0, origin, origin, base)
    assert full, "expected symbols in the 16-col region"
    # Seek to Column 0 of a row about half-way through the region (10 rows/symbol).
    row = (len(full) * 10) // 2
    start_ui = origin + row * cols
    win = swi3score.decode_symbols(sess.capture.sample_source(), cols, 0, start_ui, origin, base)

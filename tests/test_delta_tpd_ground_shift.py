...
# The swing is (V_OH − V_OL)/V_DD at those same defaults. Written as the subtraction, not as
# the literal 0.60, for two reasons: it is what the swing MEANS, and 0.80 − 0.20 is not
# exactly 0.60 in binary — so the literal would leave this oracle 1-2 ULP away from the model
# it is checking, which an exact comparison would report as a disagreement.
_SWING = 0.80 - 0.20
...
def _sigma_setup_oracle(alpha, tRF_Man, tRF_Per, shape):
    ...
    def leg(shift):
        return _t_cross_frac_rising(
            _V_IH_MAX * _VDD_PLUS + shift, _VDD_MINUS, shape, swing_frac=_SWING) * (1.0 + _TRF_TOL)
...
def test_the_rising_late_falling_early_pairing_binds_for_any_tRF_pair(shape):
    ...
    late_rise = _t_cross_frac_rising(0.65 * vp + a, vm, shape, swing_frac=_SWING) * (1 + tau)
    early_fall = _t_cross_frac_falling(0.55 * vp + a, vm, shape, swing_frac=_SWING) * (1 - tau)
    late_fall = _t_cross_frac_falling(0.35 * vm - a, vp, shape, swing_frac=_SWING) * (1 + tau)
    early_rise = _t_cross_frac_rising(0.45 * vm - a, vp, shape, swing_frac=_SWING) * (1 - tau)

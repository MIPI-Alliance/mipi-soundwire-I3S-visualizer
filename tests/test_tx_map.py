...
Run: QT_QPA_PLATFORM=offscreen PYTHONPATH=. python3 tests/test_tx_map.py
"""
import os
...
def test_tx_raster_matches_edge_into_ui():
    ...
    seg = sess.segments[0]
    cols, origin, rbase = int(seg["column_count"]), int(seg["start_ui"]), int(seg["row_base"])
    cur = max(rbase, sess.bus_row_for_sample(0))
    checked = 0
    for i in range(0, 64, 5):
        for c in range(cols):
            ui = origin + (cur - rbase + i) * cols + c
...
def test_tx_persist_columns_region_scoped_and_lights_up():
    ...
    overlay."""
    sess = Session.from_demo(64)
    # In the audio region, the transporting data columns light up (more than just CDS).
    samp = int(sess.audio_start_sample) + 5000
...
def test_persistence_relabels_rows_0_to_n():
    ...
    win._toggles_btn.setChecked(True); win._on_seek(0)
    per_row = list(seen["row_labels"])
    win._persist_btn.setChecked(True); win._on_seek(0)
...
def test_tx_virtual_scroll_pages_the_capture():
    ...
    win = MainWindow()
    win.load_demo()
    total = win._session.tx_total_rows()
    seen = {}
    orig = win._grid_view.set_tx_raster
    win._grid_view.set_tx_raster = lambda r: (seen.update(r), orig(r))
...
if __name__ == "__main__":
    test_tx_raster_shape_and_labels(); print("ok: tx_raster shape + labels")
    test_tx_raster_matches_edge_into_ui(); print("ok: tx_raster matches edge-into-UI")
    test_tx_raster_start_row_and_total(); print("ok: tx_raster start_row + total_rows")
    test_tx_raster_uses_operational_geometry_not_preamble(); print("ok: tx_raster uses operational geometry (not the preamble)")
    test_show_toggles_opens_at_cursor_row(); print("ok: Show Toggles opens at the cursor row")
    test_gui_exposes_tx_map_buttons(); print("ok: GUI exposes the TX map buttons")
    test_rows_to_draw_box(); print("ok: Rows-To-Draw box")
    test_tx_persist_columns_region_scoped_and_lights_up(); print("ok: persistence is region-scoped + lights the transporting columns")
    test_persistence_relabels_rows_0_to_n(); print("ok: persistence relabels rows 0..N-1")
    test_tx_virtual_scroll_pages_the_capture(); print("ok: TX virtual scroll pages the capture")
    print("ALL TX-MAP TESTS PASSED")

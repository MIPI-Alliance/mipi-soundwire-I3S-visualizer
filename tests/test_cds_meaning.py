...
Run: python3 tests/test_cds_meaning.py
"""
...
if __name__ == "__main__":
    test_ping_header_and_status()
    print("ok: Ping header + opcode + ping status labels")
    test_write_address_and_response()
    print("ok: WriteA32 opcode + address bytes + response labels")
    test_commit_fields_and_responses()
    print("ok: Commit group mask / row delay / per-device responses")
    test_idle_pattern_between_commands()
    print("ok: Idle Pattern between commands (spacers stay Spacer)")
    print("ALL CDS-MEANING TESTS PASSED")

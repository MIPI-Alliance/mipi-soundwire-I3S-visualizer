...
Run: python3 tests/test_regmap_import.py
"""
import os

from swi3s_studio.model.regmap_import import (
    import_text, library_dir, in_peripheral_space, PeripheralRegisterMap)
from swi3s_studio.model.registers import RegisterMap, apply_commands, Provenance
...
if __name__ == "__main__":
    test_tolerant_parse_and_counts(); print("ok: tolerant (rootless) parse + counts + address-space")
    test_coalescing_and_singletons(); print("ok: indexed-bit coalescing + singletons")
    test_field_reset_sliced_from_register(); print("ok: per-field reset sliced from register reset")
    test_no_coalesce_keeps_bits_split(); print("ok: coalesce=False keeps bits split")
    test_report_warnings_and_dropped_attrs(); print("ok: report warnings + dropped vendor attrs")
    test_json_roundtrip(); print("ok: normalized JSON round-trip")
    test_library_example_loads(); print("ok: bundled library example loads")
    test_per_device_resolve_routing(); print("ok: per-device resolve routing (peripheral vs SWI3S)")
    print("ALL REGMAP-IMPORT TESTS PASSED")

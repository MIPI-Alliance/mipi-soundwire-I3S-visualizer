...
if __name__ == "__main__":
    test_grid_diff_classes(); print("ok: grid_diff classes (same/changed/decoded_only/expected_only)")
    test_identical_grids_have_no_diff(); print("ok: identical grids -> 0 diffs")
    test_grid_diff_report_text(); print("ok: grid_diff_report summary + detail")
    test_registers_from_csv_roundtrip(); print("ok: registers_from_csv round-trips to grid_from_csv")
    test_grid_from_registers_force_columns(); print("ok: grid_from_registers force_columns overrides width")
    test_grid_from_commands_next_staging(); print("ok: grid_from_commands stages _NEXT until confirmed commit")
    test_register_diff_finds_mismatch(); print("ok: register_diff finds mismatches")
    test_grid_from_csv_optional(); print("ok: grid_from_csv")
    test_visualizer_csv_legacy_detect_and_convert(); print("ok: legacy visualizer CSV detect + upgrade to current")
    test_visualizer_csv_v2_upgrades_to_current(); print("ok: v2.0 CSV upgrades to current (adds DataPortNumber)")
    test_visualizer_csv_legacy_grid_placement(); print("ok: upgraded legacy CSV places dataports correctly")

...
Run: QT_QPA_PLATFORM=offscreen PYTHONPATH=. python3 tests/test_vcd.py
"""
import os
import tempfile
...
if __name__ == "__main__":
    test_parse_timescale(); print("ok: $timescale parsing (ns/ps/us + rejects)")
    test_read_info_signals_and_rate(); print("ok: read_info signals + sample rate (vectors excluded)")
    test_vcd_roundtrip_decodes(); print("ok: VCD round-trips to the same decode (commands + columns)")
    test_auto_clock_picks_busier_signal(); print("ok: auto-clock picks the busier signal as clock")
    test_transition_counts(); print("ok: transition_counts (clock busiest)")
    test_vcd_workspace_source_roundtrip(); print("ok: VCD workspace source round-trips + reloads")
    test_dumpoff_no_phantom_edges(); print("ok: $dumpoff x-fills don't inject phantom edges")
    test_header_comment_not_misparsed(); print("ok: header $comment with decoy tokens not misparsed")
    test_truncated_time_token_survives(); print("ok: truncated '#' time token doesn't abort the load")
    print("ALL VCD TESTS PASSED")

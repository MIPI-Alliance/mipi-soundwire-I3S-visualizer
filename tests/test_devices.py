...
Run: python3 tests/test_devices.py
"""
import os
import tempfile
...
def test_register_view_expansion_survives_rebuild():
    ...
    from swi3s_studio.model.registers import RegisterMap, DeviceRegisterFile
    rmap = RegisterMap.load()
    pm = PeripheralRegisterMap.from_json(
        open(os.path.join("data", "regmaps", "example_amp.json"), encoding="utf-8").read())
...
if __name__ == "__main__":
    test_hub_ping_decode_recovers_delayed_tokens(); print("ok: hub-delayed Ping tokens recovered with depths")
    test_hub_decode_blind_parser_garbles(); print("ok: depth-blind parser garbles delayed tokens")
    test_no_delay_is_unchanged(); print("ok: zero hub depth unchanged")
    test_decoder_settings_field(); print("ok: DecoderSettings.hub_depth_overrides")
    test_session_set_hub_depths_redecodes(); print("ok: Session.set_hub_depths re-decodes")
    test_workspace_roundtrip_device_settings(); print("ok: workspace round-trips device settings")
    test_devices_dialog(); print("ok: Devices dialog name/depth/map readback")
    test_devices_dialog_focused_sections(); print("ok: Devices dialog focused sections (names / depths only)")
    test_command_table_peripheral_register_name(); print("ok: command table shows peripheral register name")
    test_register_view_expansion_survives_rebuild(); print("ok: register-view expansion survives rebuild")
    test_register_view_compare_shows_csv_only_device(); print("ok: compare adds/removes a CSV-only device in the picker")
    print("ALL DEVICES TESTS PASSED")

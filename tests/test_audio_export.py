"""Audio store + WAV export test (device-keyed):
    ...
    with `wave` -> compare to the generated sine. The demo has 2 devices x 2 DPs.

Run: python3 tests/test_audio_export.py
"""
import math
import os
import tempfile
...
from swi3s_studio.store.audio_store import AudioStore, sign_extend, container_bits


def _expected_sine_signed(tone, index, field_bits):
    # field_bits = SampleSize register value = sample_bits - 1. `tone` is the sine
    # selector the demo uses: for device 0 / DP 0, tone == channel.
...
def test_store_shapes_and_rate():
    store = _decode_store()
    # Two devices, two stereo DPs each.
    assert store.streams() == [(0, 0), (0, 1), (1, 0), (1, 1)]
    assert store.channels(0, 0) == [0, 1]
    assert store.channels(1, 1) == [0, 1]
    assert store.rate(1, 0) > 0
    assert store.samples(0, 0, 0).size >= 16
    assert store.sample_bits(0, 0, 0) == 8
    assert container_bits(8) == 16


def test_wav_roundtrip_matches_sine():
    store = _decode_store()
    field_bits = store.sample_bits(0, 0, 0) - 1
    with tempfile.TemporaryDirectory() as d:
        paths = store.export_all(d, prefix="swi3s")
        assert len(paths) == 4              # 2 devices x 2 DPs
        ...
        with wave.open(dev0dp0, "rb") as w:
            ...
            n = w.getnframes()
            raw = w.readframes(n)
    frames = np.frombuffer(raw, dtype="<i2").reshape(n, 2)
    for ch in (0, 1):                       # dev0/dp0: tone == channel
        col = frames[:, ch]
        for i in range(min(col.size, 32)):
            exp = _expected_sine_signed(ch, i, field_bits)
            assert int(col[i]) == exp, f"ch{ch}[{i}] wav={col[i]} exp={exp}"
...
def test_export_subset_and_range():
    store = _decode_store()
    with tempfile.TemporaryDirectory() as d:
        mono = store.export_wav(1, 0, os.path.join(d, "mono.wav"), channels=[1])
        with wave.open(mono, "rb") as w:
            assert w.getnchannels() == 1
        ranged = store.export_wav(0, 1, os.path.join(d, "range.wav"), index_range=(4, 14))
...
def test_pcm_int16_for_playback():
    store = _decode_store()
    pcm, rate, nch = store.pcm_int16(0, 0)          # stereo
    assert nch == 2 and rate == int(round(store.rate(0, 0)))
    assert pcm.dtype == np.int16 and pcm.size % 2 == 0
    # Channels are interleaved (channel-major): de-interleave and check channel 0
    # tracks the channel-0 samples scaled to full-scale int16.
    bits = store.sample_bits(0, 0, 0)
    scale = 32767.0 / float(1 << (bits - 1))
    ch0 = pcm[0::2]
    src0 = store.samples(0, 0, 0)
    exp0 = np.clip(np.round(src0[:ch0.size].astype(float) * scale), -32768, 32767)
    assert np.array_equal(ch0.astype(float), exp0)
    # A windowed start drops the leading samples; mono selection -> 1 channel.
    pcm2, _r, nch2 = store.pcm_int16(0, 0, channels=[1], start=4)
    assert nch2 == 1 and pcm2.size == max(0, store.samples(0, 0, 1).size - 4)
...
def test_pcm_play_depths():
    ...
    store = _decode_store()
    b16, r16, n16, f16 = store.pcm_play(0, 0, depth=16)   # stereo
    b24, r24, n24, f24 = store.pcm_play(0, 0, depth=24)
    assert n16 == 2 and n24 == 2 and r16 == r24
    ...
    # 16-bit channel 0 matches the int16 playback scaling.
    pcm16 = np.frombuffer(b16, dtype="<i2")
    bits = store.sample_bits(0, 0, 0)
    scale = 32767.0 / float(1 << (bits - 1))
    src0 = store.samples(0, 0, 0)
    exp0 = np.clip(np.round(src0[: pcm16[0::2].size].astype(float) * scale),
                   -32768, 32767)
    assert np.array_equal(pcm16[0::2].astype(float), exp0)
...
if __name__ == "__main__":
    test_store_shapes_and_rate(); print("ok: store shapes — 2 devices x 2 DPs, per-stream rate")
    test_wav_roundtrip_matches_sine(); print("ok: WAV round-trip matches generated sine")
    test_export_subset_and_range(); print("ok: WAV export channel subset + index range")
    test_playback_pads_unequal_channels_to_max(); print("ok: playback pads unequal channels to max")
    test_pcm_int16_for_playback(); print("ok: PCM int16 for playback (interleave + scale + window)")
    test_pcm_play_depths(); print("ok: PCM playback 16/24-bit depths (format + byte width)")
    test_pcm_play_decimation(); print("ok: PCM playback decimation (resample for playback)")
    test_export_resampled(); print("ok: WAV export resampled to a target rate")

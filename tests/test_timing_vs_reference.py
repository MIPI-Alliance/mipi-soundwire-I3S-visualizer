...
def _matched_inputs(E):
    ...
    return CalcInputs(
        ...
        Man_t_ZD_ns=E.MAN_ANALOG_MAX, Man_t_DZ_max_ns=E.MAN_ANALOG_MAX,
...
def test_manager_launched_and_contention_legs_agree_exactly():
    ...
    ui = inp.UI_ns()
    zd25 = E.MAN_LAUNCH_UI * ui               # the reference's clocked turn-on placement
    ...
    exact = [
        ...
        ("setup_pm", E.setup_pm(), "MP_setup_ho", dict(Man_t_ZD_ns=zd25)),
        ("hold_pm", E.hold_pm(), "MP_hold_ho", dict(Man_t_ZD_ns=zd25)),
        ("nc_mp", E.nc_mp(release=E.man_release_conventional(E.MAN_ANALOG_MAX)),
         "MP_contention", dict(Per_t_ZD_ns=E.PER_ZD_MIN)),
        ("nc_pm", E.nc_pm(ede=False), "PM_contention",
         dict(Man_t_ZD_ns=zd25, Per_t_DZ_max_ns=E.PER_DZ_MAX)),
    ]
    ...
    # The Manager keeper leg, at the reference's EDE placements (launch 0.25 UI, release
    # 0.75 UI) — its keeper_man() is that case, not the analog one.
    ede = dict(Man_ede=True, launch_mode=LaunchMode.CLK4, Man_t_DD_min_ns=zd25,
               Man_t_DD_max_ns=zd25, Man_t_DZ_max_ns=E.MAN_RELEASE_UI * ui)
    assert _margin(inp, "keeper_Man", **ede) == pytest.approx(

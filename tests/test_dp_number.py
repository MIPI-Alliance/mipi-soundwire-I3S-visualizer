...
Run: python3 -m pytest tests/test_dp_number.py
"""
...
def test_studio_csv_roundtrip_and_stamp():
    ...
    p = _tmp()
    text = cfg.to_csv()
    # The CSV stamps the LIVE Studio version (swi3s_studio.__version__), not a literal.
    from swi3s_studio import __version__
    assert f"AppVersion,{__version__}" in text
...
def test_converter_emits_dataport_number():
    ...
    assert f"AppVersion,{app_version()}" in text

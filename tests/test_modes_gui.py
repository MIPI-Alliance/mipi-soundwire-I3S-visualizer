...
Run: QT_QPA_PLATFORM=offscreen PYTHONPATH=. python3 tests/test_modes_gui.py
"""
import os
import tempfile
...
from swi3s_studio.ui.mode_controller import VISUALIZATION, TIMING, ANALYSIS, MODES
...
if __name__ == "__main__":
    test_starts_in_visualization(); print("ok: starts in Bus Visualizer (default mode)")
    test_title_shows_capture_only_in_analyzer(); print("ok: capture name in title only in Analyzer (+ two-file bin)")
    test_commands_menu_filter_sscr_and_shortcuts(); print("ok: All excl. Ping + SSCR jump + shortcuts help")
    test_switch_swaps_central_and_docks(); print("ok: switching swaps central page + dock set + menu gating")
    test_return_restores_layout(); print("ok: returning to Analysis restores its layout")
    test_mode_persists_in_workspace(); print("ok: active mode round-trips through the workspace")
    test_authoring_places_and_checks(); print("ok: authoring places the grid + flags a clash")
    test_use_authoring_as_expected(); print("ok: 'Use as Expected Config' feeds Analysis ▸ Compare")
    test_workspace_carries_authoring(); print("ok: workspace persists the authored config + timing")
    test_dp_dialogs_apply(); print("ok: per-DP picker dialogs apply (Device/Channels/Guard/Flow/Port)")
    test_authoring_arrow_nav(); print("ok: arrow keys navigate the DP entry grid")
    print("ALL MODE-SHELL TESTS PASSED")

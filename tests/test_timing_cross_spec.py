...
# The P->P DATA legs are newer than the proposal, and the proposal explicitly scoped P->P
# communication out ("not guaranteed"), so a claim of the form "the proposal closes every
# inequality" is a claim about the GUARANTEED directions. Filtering them here rather than
# weakening each assertion to `most` keeps that claim exact; what the PP legs actually read
# is recorded by the tests named for them, which is where a change to them should show up.
_PP_DATA_LEGS = ("PP_setup", "PP_setup_ho", "PP_hold", "PP_hold_ho")
...
def test_the_ede_column_pair_is_proposal_versus_ede():
    ...
    # Right column = EDE: a clocked Manager launch collapses Man_t_DD to a point.
    lo, hi = (tv._min[("proposed", "Man_t_DD")].value(),
              tv._max[("proposed", "Man_t_DD")].value())
    assert lo == hi, "a clocked launch is deterministic — min must equal max"

    # EDE asserts the flags on the CANDIDATE column only; the baseline has no EDE.
    assert tv.inputs("proposed").Man_ede and tv.inputs("proposed").Per_ede
...
def test_ede_helps_every_leg_it_owns():
    ...
    * Which legs reach positive is a RESULT, and is recorded as one rather than
    ...
    # The mechanism: every contention leg is better with EDE than without, and the
    # baseline fails all three, so the feature is doing real work on each.
    for leg in ("MP_contention", "PM_contention", "PP_contention"):
        assert base[leg].margin_ns < 0, f"{leg}: the baseline should fail at zero UIs"
        assert ede[leg].margin_ns > base[leg].margin_ns, \
            f"{leg}: EDE must improve the leg its releaser owns"
    ...
    assert ede["keeper_Man"].margin_ns > 0, ede["keeper_Man"].margin_ns
...
def test_the_ede_ui_fraction_rows_follow_the_clock_rate():
    """Man_t_DZ is "0.75 UI", so an absolute value carried across a rate change is
    not stale-but-usable, it is wrong at both rates.

    Narrowly scoped: the Manager's UI-fraction rows refresh, and an edit to an
    ABSOLUTE row survives -- the reseed trap `_on_launch_changed` documents.
    Per_t_DZ is no longer among them: the peripheral's EDE release is an interval
    from its own ramp completion, not a fraction of the UI.
    """
    tv = _view()
    for combo in (tv._man_spec_combo, tv._per_spec_combo):
        combo.setCurrentIndex(combo.findData("SWI3S_PHY2_EDE"))
    tv._max[("proposed", "Per_t_ZD")].setValue(7.5)      # absolute row, edited
    for f in (12.288, 9.0, 6.0):
        tv._fclk.setValue(f)
        ui = 1000 * 0.45 / f
        assert tv._max[("proposed", "Man_t_DZ")].value() == pytest.approx(
            0.75 * ui, abs=0.05), f
        assert tv._max[("proposed", "Man_t_DD")].value() == pytest.approx(
            0.25 * ui, abs=0.06), f      # one-decimal spin box: 18.75 shows as 18.8
...
def test_the_keeper_needs_the_release_two_grid_steps_after_the_launch():
    ...
    This test used to assert that selecting 4x "recovers" the keeper, because the
    mode pinned Man_t_DD to 0.25 UI and that happened to leave room before a
    mid-UI release. The mode no longer supplies a placement -- it supplies the
    grid -- so the claim has to be made about the thing that actually decides it:
    t_DZ must clear t_DD by the whole rail-to-rail swing PLUS the keeper's response
    time, 13.83 + 3.00 = 16.83 ns at the slow corner (0.46 UI), which on a 1/4 UI grid
    is TWO steps.

    The docstring said "at least t_keeper after t_DD, which on a grid means a LATER
    POINT" -- the retired swing-for-t_keeper reading, and it admits ONE step, which the
    body below has always asserted fails. The inline comments were right and the
    docstring was not.
    """
    tv = _view()
    for combo in (tv._man_spec_combo, tv._per_spec_combo):
    ...
    def keeper_with(dd, dz):
        for name, v in (("Man_t_DD", dd), ("Man_t_ZD", dd), ("Man_t_DZ", dz)):
            tv._min[("proposed", name)].setValue(v)
            tv._max[("proposed", name)].setValue(v)
        return tv._per_inequality_worst("proposed")["keeper_Man"].margin_ns

    assert keeper_with(0.50 * UI, 0.50 * UI) < 0, \
        "release on the launch's own point must fail -- no driven valid level"
    # ONE grid step is NOT enough, and that is what sets the multiplier. The addend
    ...
    # TWO steps do, which is why EDE needs a 4x clock: a 2x clock has no pair of
    # points two 0.25 UI steps apart with the later one still inside the UI.
    assert keeper_with(0.25 * UI, 0.75 * UI) > 0, \
        "two grid steps must satisfy it -- 0.50 UI covers the 0.3777 UI swing"
    # AND THE BOUNDARY IS NOT REACHABLE, which is the point Table 130's ceiling makes.
    # It used to satisfy the keeper generously (+10.63 from a 0.50 UI launch) and that
    # was a false pass: 1.00 UI is outside 0.60 UI + 10 ns and is not an early release.
    # The model now pulls it back to the latest legal grid point -- 0.75 UI here -- which
    # from a 0.50 UI launch is ONE step, so the leg fails and says so.
    assert keeper_with(0.50 * UI, UI) < 0, \
        "the boundary must be capped to 0.75 UI, one step from a 0.50 UI launch"


def test_ede_needs_a_quarter_UI_grid_and_is_seeded_for_it():
    """EDE needs a 1/4 UI grid -- a 4x clock -- and the reason is structural.

    Three constraints must be met by DISTINCT grid points once every Manager output
    is clocked and deterministic:

        hold on either launch method   t_DD = t_ZD  >= ~10 ns
        the keeper                     t_DZ - t_DD  >= swing + t_keeper = 16.83 ns
        Table 130                      t_DZ         <= 0.60 UI + 10 ns

    A 1/2 UI grid offers 0.00, 0.50 and 1.00 UI. Its single interior point cannot
    be both the launch and the release; 0.00 fails hold by ~10 ns; and 1.00 UI is
    the boundary, which Table 130 forbids and which is not an early release anyway.
    A 1/4 UI grid adds 0.25 and 0.75, and t_DD = t_ZD = 0.25 UI with t_DZ = 0.75 UI
    satisfies all three -- 18.31 ns of separation against the 16.83 the keeper needs.

    THE KEEPER IS NOT WHAT RULES 2x OUT, and the constraint list said it was: with
    `>= t_keeper` it read as 3 ns, which 0.50 -> 1.00 UI clears easily, and it named
    a 0.50 UI LAUNCH as the pairing that works -- the placement retired when the
    keeper's addend became the full swing, and the one the test above proves fails.
    A 1/2 UI grid does contain a pair 16.83 ns apart; Table 130's ceiling is what
    excludes it.

    This test asserted the opposite until 2026-08-06 -- that 2x was the
    recommendation -- and that followed from giving t_DZ a 3-9 ns analog turn-off on
    top of its grid point, which let it sit 3 ns after a 0.50 UI launch on a grid
    that cannot place such a point. An analog delay on top of a grid launch is
    possible and would be the fallback if no fully clocked configuration existed;
    one does, so it is not used.
    """
    tv = _view()
    for combo in (tv._man_spec_combo, tv._per_spec_combo):
        combo.setCurrentIndex(combo.findData("SWI3S_PHY2_EDE"))
    ...
    # The seeded placements, and every one deterministic -- no analog term anywhere.
    for name, frac in (("Man_t_DD", 0.25), ("Man_t_ZD", 0.25), ("Man_t_DZ", 0.75)):
        lo = tv._min[("proposed", name)].value()
        hi = tv._max[("proposed", name)].value()
        assert lo == pytest.approx(hi), f"{name} must be deterministic when clocked"
        assert lo == pytest.approx(frac * UI, abs=0.06), (name, lo, frac * UI)

    # The release is EARLY, which is the whole feature, and by a quarter UI.
    dz = tv._max[("proposed", "Man_t_DZ")].value()
    assert UI - dz == pytest.approx(0.25 * UI, abs=0.06), \
        "the release must precede the UI boundary, or EDE is not doing anything"
    # And inside Table 130's stated maximum.
    assert dz <= 0.60 * UI + 10.0

    w = tv._per_inequality_worst("proposed")
    short = [k for k, b in w.items() if b.margin_ns < -0.005]
    # EDE closes every leg it OWNS -- the three contention legs and both keeper
    # legs. It does not close the two MP hold legs, and those are not its to close:
    # they are the ordinary legs, and MP_hold carries no t_ZD at all, so EDE cannot
    # be what moves them. Both sit at about -0.7 ns on the revision's own
    # Man_t_DD,min / Man_t_ZD,min of 9 ns against a floor of 9.89 at the slow
    # corner, which is the residual the underlying revision already carries.
    #
    ...
    # -2.38 ns), and neither move cost a red test while the bound was one-sided. The
    # membership is the same as it was; what is asserted about it is not.
    ...
    # What the 0.25 / 0.75 placement is chosen FOR: the Manager-releasing leg and
    # both keeper legs. Those are what the grid decides. The peripheral's keeper leg sits
    # at exactly zero under EDE (its hold minimum IS Man_tKeeper_Response), so it is
    # asserted as closed-with-nothing-spare rather than as positive.
    for leg in ("MP_contention", "keeper_Man"):
        assert w[leg].margin_ns > 0, (leg, w[leg].margin_ns)
    ...
    # A 1/2 UI GRID CANNOT EXPRESS IT, and the model has to SAY so rather than leave it
    # to a comment. It used to round the 0.75 UI seed up to the boundary and display
    # 1.00 UI, where every leg reads green -- MP contention +3.48, the keeper +1.48 --
    # so the tool reported 2x as a working EDE design. Table 130's ceiling pulls the
    # release back to the latest legal point a 1/2 UI grid can place, which is 0.50 UI:
    # the launch's own point, no driven valid level before the release, keeper fails.
    tv._launch_combo.setCurrentIndex(tv._launch_combo.findData(LaunchMode.CLK2.value))
    dz2 = tv._max[("proposed", "Man_t_DZ")].value()
    assert dz2 == pytest.approx(0.50 * UI, abs=0.06), \
        "Table 130 must cap the 2x release at 0.50 UI, not round it to the boundary"
    assert dz2 <= 0.60 * UI + 10.0, "the displayed release must be legal EDE"
    w2 = tv._per_inequality_worst("proposed")
    assert w2["keeper_Man"].margin_ns < 0, \
        "which is the point: 2x cannot do EDE, and now it fails instead of reading green"
    ...
    # No claim about the SIZE of the gap: it was ~0.7 ns when this was written and the three
    # rewrites above are what a pinned magnitude here would have cost. The ordering is the
    # load-bearing statement.
    assert mp > 10.0, "M->P is the comfortable one: the crossing is a credit there"
...
def test_ede_peripheral_values_are_not_tighter_than_3_to_1():
    ...
    # EVERY Manager output is a clocked grid point on this column, so every one of
    # them must be DETERMINISTIC -- min == max, ratio 1:1. There is no "analog
    # portion" to carve out.
    #
    # An earlier version of this test did carve one out: it subtracted an assumed
    # 0.50 UI grid point from Man_t_DZ and checked only the remainder, which came to
    # 9/3 = 3:1 and passed. That made the check UNFALSIFIABLE -- the test chose
    # which part of the value to excuse, so any range could be made to pass by
    # nominating a suitable grid point. Man_t_DZ was 21.31-27.31, i.e. 1.28:1 as a
    # whole value, and the test waved it through. Rod caught it by inspection.
    #
    # The rule that replaces it: a clocked value is deterministic and 3:1 does not
    # apply; an unclocked value carries 3:1 over the WHOLE of itself. Nothing in
    # between, and no subtraction.
    for name in ("Man_t_DD", "Man_t_ZD", "Man_t_DZ"):
        assert ratio(name) == pytest.approx(1.0), \
            f"{name} is {ratio(name):.2f}:1 — a clocked Manager output must be " \
            f"deterministic, and a range here means it is not clocked after all"

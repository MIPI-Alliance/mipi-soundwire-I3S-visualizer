...
Run: python3 -m pytest tests/test_symbols.py
"""
import swi3score
...
def _symbols(n=8, maxn=60):
    # The demo starts in safe-lock-2 (2-column control), so frame the leading Ping at
    # 2 columns — the geometry widens to 8 then 16 only after the first commits.
    return swi3score.decode_symbols(transitions.demo_capture(n).sample_source(), 2, maxn)

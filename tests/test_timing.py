...
def test_anchored_terms_keep_the_palette_ink_in_both_modes():
    ...
    from swi3s_studio.ui.timing_view import _anchored, _subscript
    ...
    for mode in ("dark", "light"):
        ...
        def ink(html):
            ...
            # The delegate's own stylesheet, with nothing highlighted.
            doc.setDefaultStyleSheet(
                f"a {{ color: {theme.VizTheme.TEXT}; text-decoration: none; }}")
            doc.setHtml(html)
        ...
        assert theme.VizTheme.TEXT in got, (
            f"{mode}: anchored text is not drawn in the palette's ink -- got {sorted(got)}")
        assert got == want, f"{mode}: anchoring changed the ink: {sorted(want)} -> {sorted(got)}"
        assert "#0000ff" not in got, f"{mode}: anchors are being drawn as links"

...
- `tests/visualizer/golden/<category>/<name>.txt` — the visualizer's raw
  per-data-port placement for that config (generated from the visualizer's
  `model_dump.py`), one `dp row col SLOT` line per owned slot, with a `# rows N`
  header.
...
Run: PYTHONPATH=. python3 tests/test_visualizer_placement.py
"""
import glob
import os
import tempfile
...
if __name__ == "__main__":
    test_visualizer_examples_present()
    print(f"ok: 89 visualizer example configs vendored")
    test_placement_matches_visualizer()
    print("ok: Studio per-DP placement matches the visualizer for all 89 configs")
    print("ALL VISUALIZER PLACEMENT TESTS PASSED")

...
#        one fixes the second of those defects, tools and documents the VM run so the step
#        cannot be dropped again, and was gated on the Windows guest (Python 3.14) BEFORE
#        being tagged. No product change beyond the dialog layout.
#        • THE VM GATE IS NOW A CHECKLIST STEP WITH A RUNNER, because the checklist kept
#          losing it and that cost two releases. The gate on a test VM is now step 3 of the
#          release checklist, BEFORE tagging, and it is scripted rather than remembered: the
#          runner ships `git archive <sha>` to a guest, extracts it BESIDE the working copy
#          (neither guest is a git checkout, so a fetch there does nothing and overwriting
#          would destroy work in flight), runs `tools/gate.py` against the guest's own venv,
#          and leaves the log ON the guest. The setup notes carry every trap that was paid
#          for: a hidden launcher's output redirection wrote 0-byte logs twice; piping a
#          guest's output home dies on macOS `sed` with "illegal byte sequence" and takes a
#          12-minute run with it; PowerShell's append redirection writes UTF-16 so the log
#          reads as binary unless NULs are stripped; and a killed local pipeline leaves the
#          gate running on the guest with its output going nowhere. The runner and the notes
#          live in the maintainers' tier, since their subject is another machine — which is
#          also why they are described here rather than linked.
...
#          AND THE CASE FOR AN UBUNTU GUEST IS RECORDED with the one thing that decides
#          whether it is worth having: ci.yml's Linux jobs install NO FONTS, so a desktop
#          Ubuntu guest would pass rendering tests CI fails and recreate the blind spot it
#          exists to remove.
...
#          Rod's, and the scoping question that decided the design was what "the same term"
...
#          `Man_t_RF,DATA` selected `Per_t_IS` two terms along — Rod's report. The option now
...
#          accent the chrome already uses. An accent-derived wash was tried first and Rod
#          judged it too subtle, which the side-by-side render made obvious. `HILITE_BG` is
...
#        • EVERY LEG NOW READS IN PHYSICAL TIME ORDER, which is Rod's and is the change that
...
#          in `summary_text()`. Rod's, for reference: naming a leg in conversation took
...
#          still agree. ")" AND NOT ".", also his: the equations are full of the "·" operator
#          and decimals like "1.44·8.30", so a leading "1." reads as arithmetic.
...
#          "Peripheral A Holding for Peripheral B". Rod's wording, including the ruling that
#          setup reads "Setup" rather than "Launching": the launch is a step on the way and
#          the requirement is the goal. The P→P pair carries the same letters its terms
...
#          READ THAT SIGN CORRECTLY, and this is Rod's note rather than a derivation: on
...
#          Rod's, and it corrected my diagnosis twice: I first called the collapse a
#          consequence of having "one row", which is wrong — a row is a BAND and each part
#          draws from it — and then proposed that a pick should "shift the window", which is
#          incoherent, since the window is the spec limit and there is nothing outside it to
#          shift into. The fix is simply not to perturb it.
...
#            lateness and the Manager cannot spend it by releasing later. Rod's question,
#            and he was right: at the boundary the leg reads +10.10 and every nanosecond
#            of that is the peripheral being unable to start at the edge. The leg goes
#            negative only WITHOUT EDE, where the release moves 23 ns PAST the closing
...
#          made to pass by nominating a suitable grid point. Rod caught it by inspection.
...
#          is the correct state and a reader should not re-derive the wrong one. I removed
...
#          value. Removing the back-out made them differ by t_RF/3, which is how Rod caught
#          it. Reverted; PM_setup_ho is back to +6.185 and MP hold is again the only short
#          leg at the all-extremes corner. Per and Man differ here by anchor symmetry
#          (Figures 174 vs 176), which is consistency per side, not a disagreement — I had
#          misdiagnosed that too.
#        • THREE "the value on screen is not the value in force" defects, two reported by Rod
...
#            two was my own second error on the same fix. A 4× clock offers 0.00, 0.25, 0.50
...
#          12.288 design point. That is the finding, not a defect, and my first write-up had it
#          backwards: I read it as failing against Table 126's mandatory 13.2 MHz and
#          recommended tightening Per_t_DD,max to 8.36 ns to reach it. 13.2 predates the timing
...
#          reference, so it did not have to go; taking
#          them is a concession to a reviewer who was right about the larger point and had
#          been pushed back on for it three times. The polarity inversion came back in his
#          own shape rather than ours — `"""PortDirection_REG=True means SINK"""` is the
...
#          Both findings this cycle came from Rod reading the model rather than from a test:
...
#          had. Caught by Rod on a real config, not by a test: all 89 corpus examples use
...
#          above is closed. His _advance_channel now sizes the trailing group with a `min`
#          against the remaining enabled channels — the partial-channel-group fix, in his
#          own shape. Taken as authored. What it also drops is the `max(0, ...)` around the
#          seed, and that is safe: the transport-window gate already requires

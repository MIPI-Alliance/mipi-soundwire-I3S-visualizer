...
# 3.0.18: (in development).
#        The header stays "(in development)" until the release commit, and the version does not
#        change again during the cycle — see docs/DEVELOPMENT.md, "A corp push is a checkpoint,
#        not a release". Pushing this branch to the corp remote is a checkpoint: no tag, `main`
#        untouched. The multi-platform gate and the single tag come at the END, once.
...
#          model" — the same disagreement the UI's menu item still carries. Behaviour follows v2
...
#          invisible because nothing was worth passing until there was a CLI. The first real use
#          of `-o` started the UI and wrote no file, which reads as the new option being broken
#          rather than the launcher. Both now forward, each still consuming its own rebuild flag;
#          `run.ps1` also propagates the exit code, which PowerShell drops without an explicit
...
#          this platform, so it keeps the text guards and that is a NOT-RUN-HERE gap rather than
#          a pass — the Windows guest is where it gets exercised.
#        • `publish_tree.py --preview` PUBLISHES AN UNFINISHED CYCLE, saying so. The
#          release-notes check refuses a file still marked "(in development)", which is right for
#          a release and wrong for a preview — and a preview branch is otherwise
#          indistinguishable from a release branch: same shape, same assembly, same subject. So
#          `--preview` lifts that one refusal and in exchange marks the subject line and leads
#          the body with what it is: not a release, not tagged, not through the multi-platform
#          gate. The refusal names `--preview` too, since a check that only says no invites
#          deleting the marker instead.
#          Also fixed on the way: `tools/leak_scan.py` tripped publish_tree's dangling-reference
#          check, because 3.0.18 gave it a prefix for the maintainers' tier of its own. That is
#          the tier BOUNDARY, not a link to a document, so it joins the same self-exemption the
#          scanner and the reference-model guard already have — and a test already pins that the
#          scanner and the pruner agree about where the boundary is. (This entry may not spell
#          the path either: the changelog ships, so writing it here is the same dangling
#          reference one file over.)
...
#          equally unrunnable. The Windows launcher had the same defect — asked about, checked,
...
#          at 1500 opportunities PHY1 transports 1380 and PHY2/PHY3 1379. A first version of the
#          test helper returned the single-run figure and would have been wrong for PHY1 by one.
...
#          0.04% (25.58 GB against 25.57 GB).
...
#          was used and a degraded one warns once in the log. The first version of the ordering
#          test asserted free > total > unknown across three DIFFERENT figures and failed —
#          correctly, because that claim is false: total RAM cannot see occupancy, so on a
#          mostly-full machine the total-derived budget legitimately exceeds the free-derived
#          one. What is true, and now asserted, is that the same figure through a weaker rule
#          yields a smaller budget.
#        • PUBLISHED PROSE IS NOW GOVERNED, AND PARTLY CHECKED. v3.0.17's public branch
#          description named a person, two capture files and the maintainers' test platforms,
#          and every leak check passed — because the scanner reads FILES (a description is not
#          one) and carries only STRUCTURAL patterns (a name is prose). Verified by putting the
#          old text in the tree as a tracked file: still "no leaks found".
#          Three things follow. `docs/DEVELOPMENT.md` gains "Write the RULE, not the story" —
#          no discovery narrative, generalise the artifact, generalise the finding, keep the
#          measurement and drop the label saying whose it was. `tools/leak_scan.py --text=`
#          scans a release description or commit message, closing the artifact half.
#          And site patterns can now be marked `published-only:`, which is what made the
#          capture names addable at all: they belong in the scan for what SHIPS while remaining
#          legitimate in the maintainers' internal tier, where the format notes cite the captures
#          a decode was verified against. Without that distinction the names could only have
#          been added by failing the gate on an internal record, which is why they never were.
#          Four shipped files and the 3.0.17/3.0.18 changelog entries are de-narrated; the
#          scanner and the pruner now agree about the tier boundary by test, not by comment.
#        • THE WHOLE CHANGELOG IS SWEPT, and enabling the patterns is what proved the rule
#          needed narrowing. 33 references across five earlier entries: 18 attributions of a
#          finding to a person and 15 pieces of test-machine detail. Attribution is gone
#          everywhere — the substance and every measurement kept, and "reported"/"raised from
#          the bus side" retained where it distinguishes a bus-behaviour claim from a
#          derivation, which is a technical distinction rather than a social one. The
#          second-platform write-ups are compressed to what a reader can act on: that the gate
#          runs on every configured platform before tagging, one command, and that an
#          unconfigured platform reports UNTESTED rather than passing. The mechanism it used to
#          describe — how the tree is shipped, where the log lands, which shell mangles which
#          encoding — is tooling detail about other machines and is not release news.
#          THE FIRST PATTERN SET WAS TOO BLUNT AND IS NARROWED. Flagging the word for a virtual
#          machine caught `docs/DEVELOPMENT.md` describing the release process on purpose, with
#          identifiers deliberately withheld — a contributor needs that description, so the
#          pattern was wrong, not the document. What actually leaked was NARRATING which machine
#          found what, and that is a style rule; the scanner keeps the two terms with no
#          legitimate use in published prose. Three more real hits surfaced on the way (two
#          shipped tests attributing a build failure to a person, one comment in the gate), and
#          the guidance itself had to stop spelling out the vocabulary it forbids — the same
#          self-matching trap the scanner avoids by building its patterns from parts.
#        • AND THE SWEEP ITSELF NEEDED PROOFREADING, which is now a rule for mechanical sweeps
#          over prose. 33 substitutions left a doubled article, two dangling objects, a pronoun
#          with no antecedent and a 147-character line — through a green gate and a green leak
#          scan, both correctly, since neither can see whether a sentence still parses to a
#          human. The residue the keyword pass could not reach went with it: three pronouns
#          standing in for individuals, and four first-person accounts of how a finding was
#          reached. `docs/DEVELOPMENT.md` gains the rule under "Mechanical sweeps" — read every
#          changed line AND its neighbours, because a replacement's fallout lands next to it.
#        • THE SCANNER REPORTED PASSES ABOUT INPUT IT HAD NOT READ, in two ways, both found by
#          using it on a real publish rather than by testing it. An unrecognised option was
#          IGNORED: the parser took the first argument not starting with "-" as the revision, so
#          `--tex=` instead of `--text=` left both unset, fell through to the default, scanned
#          the working tree and printed "no leaks found" — about a file it never opened. Now
#          exit 2, and a named file that does not exist is also exit 2 rather than an empty
#          read. That is the NOT-RUN-HERE principle in the one tool whose whole job is to be
#          believed.
#          AND ITS SELF-EXEMPTION WAS PER FILE WHERE IT SHOULD BE PER PATTERN KIND. The scanner
#          and its test skip the STRUCTURAL patterns because they hold them as literal source —
#          but they were skipped ENTIRELY, so the site-specific patterns never reached them
#          either. The site list lives outside the repository, so it cannot self-match by
#          construction, and the moment those patterns were let in they flagged a real leak in
#          the scanner's own docstring, shipped for a release: it described what a published
#          description had disclosed by repeating the vocabulary, in the one file guaranteed to
#          escape the scan. Both guards are mutation-tested — restore either behaviour and a
#          test reddens.
#        • A RELEASE IS NOW DESCRIBED ONCE, in `docs/releases/vX.Y.Z.md`. v3.0.17 was described
#          THREE times by hand — a signed tag annotation, a corp release body and the publish
#          branch's commit message — from a changelog that was already the real record. They
#          disagreed, and one ANNOUNCED A FIX THAT LANDED AFTER THE TAG: the notes were written
#          at push time, by which point the psutil-fallback work existed and the sentence read
#          as true. Every consumer now reads the one file, so they cannot drift from each other,
#          and because the file is IN THE TREE it is in the tag, appears in the release diff,
#          and the leak scan covers it as an ordinary tracked file with no `--text=` to
#          remember. `publish_tree.py` refuses a tree whose version has no notes, or whose notes
#          are still marked "(in development)", and `--message-out=` writes the branch message
#          as a DERIVED preamble (the prune and graft counts, which it knows for a fact) plus
#          the file verbatim — nothing retyped, since a retyped body is what drifted. Five tests
#          pin it: the notes exist and name this version, they reference no LATER version except
#          under an explicit "Not in this release" heading, and the tool's refusal and its
#          message derivation are asserted behaviourally rather than by grep. v3.0.17 gets its
#          notes retroactively, so the mechanism starts from a release that has one. What does
#          NOT belong in the file is anything about the publish mechanism — the tool knows that
#          and generates it. The tag annotation cannot be generated, so it becomes a POINTER:
#          one paragraph and a reference to the notes. v3.0.17's was rewritten to that shape,
#          which also removed the last 3.0.17 text carrying development-environment narrative —
#          two site patterns fired on it, and nothing had ever scanned it, because an annotation
#          is not a file. REWRITING AN ANNOTATION IS NOT THE 3.0.6 SMELL: that was moving a tag
#          to a different COMMIT, which says the release was cut too early. The target here did
#          not change. The real cost is that `git fetch` does not update a tag already held
#          locally, which is itself the argument for an annotation with nothing in it to drift.

...
#
# THE PP DATA LEGS ARE NEW, and they reverse a deliberate omission. A peripheral
...
class CalcResults:
    ...
    # THE HEADLINE RATE IS THE GUARANTEED BUS'S RATE, so the P->P data legs are excluded
    # from it while keeping their own rows and margins.
    #
    # They are not a guaranteed requirement -- the spec does not promise that one peripheral
    # can read another's data, and these legs exist to PRICE that promise rather than to
    # impose it. Left in, a failing P->P hold pins F_max_binding to 0.0 for every
    # configuration and names itself the binding constraint, which would report "no clock
    # rate works" for a bus whose guaranteed directions are fine. The two P->P legs report
    # their own margins, and their own F_max where they consume a UI, so nothing is hidden --
    # what is excluded is the claim that they bound the BUS.
    #
    # If P->P ever becomes guaranteed, delete `_PP_DATA_LEGS` and this note with it.

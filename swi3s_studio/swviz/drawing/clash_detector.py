...
from ..models.bus_model import ClashType as DeviceClashType
...
class ClashDetector:
    ...
    def _get_device_for_type(self, bit_slot: int, slot_type: SlotOccupancyType) -> Optional[int]:
        ...
        Returns the FIRST matching occupant only. Safe for WRITE/TAIL, which
        the engine only ever adds once per (bit_slot, device). GUARD and
        HANDOVER can have several occupants at once now that the CDS region
        is per-source (Manager + up to 12 devices sharing one guard/handover
        column) — use _get_devices_for_type for those.
    ...
    def _get_devices_for_type(self, bit_slot: int, slot_type: SlotOccupancyType) -> List[int]:
        """Get ALL device numbers with a given occupancy type at a slot.

        The CDS guard/handover columns are shared across sources (Manager and
        each device may each occupy the same column), so callers that need to
        clash-check a write/handover against "any existing occupant" must
        consider every occupant, not just the first.
        ...
        Returns:
            List of device numbers (possibly empty) with that occupancy type.
        """
        if bit_slot not in self.occupancies:
            return []
        return [occ.device for occ in self.occupancies[bit_slot] if occ.slot_type == slot_type]
    ...
    def check_write_clash(self, row: int, column: int, device: int) -> Tuple[bool, bool, Set[str]]:
        ...
        Returns:
            Tuple of (has_clash, should_remove_handover, suppress_same_device_slots)
            - has_clash: True if there's a write clash
            - should_remove_handover: True if handover from same device should be removed
            - suppress_same_device_slots: set containing 'guard' and/or 'tail' for each
              same-device slot the data bit should override (a single column can carry
              both a same-device guard and tail), empty set otherwise
        """
        bit_slot = self._get_bit_slot(row, column)
        has_clash = False
        should_remove_handover = False
        suppress_same_device_slots: Set[str] = set()
        ...
        if self._has_occupancy(bit_slot, SlotOccupancyType.WRITE):
            ...
            self.logger.warning('Bus clash detected (write)', extra={
                'row': row,
                'column': column,
                'device': device,
        ...
        # Check handover - a CDS handover column may have several occupants
        # (one per driving source). Same-device occupants are removed
        # (reconciled); a different-device occupant clashes.
        for handover_device in self._get_devices_for_type(bit_slot, SlotOccupancyType.HANDOVER):
            if handover_device != device:
                ...
                self.logger.warning('Bus clash detected (write vs handover)', extra={
                    'row': row,
                    'column': column,
                    'device': device,
        ...
        # Check guard - a CDS guard column may have several occupants (one
        # per driving source). Same-device occupants are suppressed (data bit
        # has priority); a different-device occupant clashes.
        for guard_device in self._get_devices_for_type(bit_slot, SlotOccupancyType.GUARD):
            if guard_device == device:
                # Same device - data bit has priority, suppress guard
                suppress_same_device_slots.add('guard')
        ...
        # Check tail - a CDS tail column may have several occupants (one per
        # driving source). Same-device occupants are suppressed (data bit has
        # priority); a different-device occupant clashes.
        for tail_device in self._get_devices_for_type(bit_slot, SlotOccupancyType.TAIL):
            if tail_device == device:
                # Same device - data bit has priority, suppress tail
                suppress_same_device_slots.add('tail')
        ...
        return has_clash, should_remove_handover, suppress_same_device_slots
    ...
    def check_guard_clash(self, row: int, column: int, device: int,
                          is_cds: bool = False) -> Tuple[bool, int, bool]:
        ...
        Args:
            ...
            column: Column number
            device: Device number
            is_cds: True when this guard is in the CDS (column 0) region,
                where the guard column is time-multiplexed across the Manager
                and up to 12 devices — see the guard-vs-guard check below.
                False (default) for a per-data-port guard (DataPort
                GuardEnable_REG/GuardPolarity_REG), which is a single
                exclusive occupancy like write/tail/handover.
        ...
        # Check handover from same/different device. A CDS handover column may
        # have several occupants (one per driving source): a same-device
        # occupant will be removed by the caller (guard takes its place); a
        # different-device occupant clashes.
        handover_devices = self._get_devices_for_type(bit_slot, SlotOccupancyType.HANDOVER)
        if handover_devices:
            if device in handover_devices:
                ...
                # Different device(s) - clash
                self.write_clashes.add(bit_slot)
                for handover_device in handover_devices:
                    self.record_clash(bit_slot, SlotClashCategory.WRITE_CLASH,
        ...
        # Check guard clash. In the CDS, guards are time-multiplexed: the
        # Manager and each device can drive their own guard polarity in the
        # same shared CDS guard column without a bus clash (unlike
        # write/tail/handover occupancies, which are exclusive) - so
        # guard-vs-guard never clashes there, same device or different, and
        # this source's occupancy is just added alongside the existing
        # one(s). Outside the CDS (a per-data-port guard), the guard column
        # is exclusive - same-level guards always clash, even same device.
        ...
        # Check tail - guard takes priority within same device. A CDS tail
        # column may have several occupants (one per driving source).
        tail_devices = self._get_devices_for_type(bit_slot, SlotOccupancyType.TAIL)
        if tail_devices:
            if device in tail_devices:
                ...
                self.write_clashes.add(bit_slot)
                for tail_device in tail_devices:
                    self.record_clash(bit_slot, SlotClashCategory.WRITE_CLASH,
    ...
    def check_tail_clash(self, row: int, column: int, device: int,
                         is_cds: bool = False) -> Tuple[bool, int]:
        ...
        Args:
            ...
            column: Column number
            device: Device number
            is_cds: True when this tail is in the CDS (Column-0) region, where the
                CDS is time-multiplexed — the Manager and each device may drive their
                own tail in the shared CDS tail column without a bus clash (like the
                CDS guard; see check_guard_clash). So tail-vs-tail never clashes there.
                Tail-vs-write / tail-vs-handover with a different device still clash.
        ...
        # Check handover from same/different device. A CDS handover column may
        # have several occupants (one per driving source).
        handover_devices = self._get_devices_for_type(bit_slot, SlotOccupancyType.HANDOVER)
        if handover_devices:
            if device in handover_devices:
        ...
        # Check tail clash. In the CDS, tails are time-multiplexed like guards:
        # several sources (Manager + devices) may each drive their own tail in the
        # shared CDS tail column without a bus clash, so this source's occupancy is
        # just added alongside. Outside the CDS, same-level tails always clash.
        ...
        # Check guard - guard has priority, don't replace. A CDS guard column
        # may have several occupants (one per driving source); any of them
        # being the same device is enough to give guard priority (no clash),
        # otherwise it clashes against all of them.
        guard_devices = self._get_devices_for_type(bit_slot, SlotOccupancyType.GUARD)
        if guard_devices:
            if device in guard_devices:
                # Same device - guard has priority, don't draw tail
                return False, 0
            else:
                self.write_clashes.add(bit_slot)
                for guard_device in guard_devices:
                    self.record_clash(bit_slot, SlotClashCategory.WRITE_CLASH,
    ...
    def check_read_clash(self, row: int, column: int, device: int) -> bool:
        ...
        if self._has_occupancy(bit_slot, SlotOccupancyType.READ):
            ...
            self.logger.warning('Read overlap detected', extra={
                'row': row,
                'column': column,
                'device': device,
        ...
        if self._has_occupancy(bit_slot, SlotOccupancyType.HANDOVER):
            ...
            if handover_device is not None and handover_device != device:
                ...
                self.logger.warning('Read overlap detected (vs handover)', extra={
                    'row': row,
                    'column': column,
                    'device': device,
    ...
    def add_handover(self, row: int, column: int, device: int,
                     canvas_ids: List[int], is_cds: bool = False) -> Tuple[bool, bool, bool]:
        ...
        Args:
            ...
            is_cds: True when this handover is a CDS (Column-0) system turnaround.
                The CDS is time-multiplexed: a guard-only source hands the bus over
                right after its own guard, in a column where a *different* source may
                still be driving its (longer) tail/guard. Those coexist — a
                different-source CDS tail/guard neither clashes with nor suppresses
                this handover. A different-device data-port WRITE in this column
                still clashes (that's a real bus collision), and a same-device slot
                still suppresses the handover.
        ...
        if self._has_occupancy(bit_slot, SlotOccupancyType.READ):
            read_device = self._get_device_for_type(bit_slot, SlotOccupancyType.READ)
            self.read_clashes.add(bit_slot)
            self.logger.warning('Read overlap detected (handover)', extra={
                'row': row,
                'column': column,
                'device': device,
                'existing_device': read_device
        ...
        if self._has_occupancy(bit_slot, SlotOccupancyType.WRITE):
            ...
            if write_device != device:
                ...
                self.logger.warning('Bus clash detected (handover)', extra={
                    'row': row,
                    'column': column,
                    'device': device,
                    'existing_device': write_device
        ...
        # A CDS tail column may have several occupants (one per driving source).
        # When is_cds, a *different* source's tail is time-multiplexed with this
        # handover (a guard-only source hands over where another source still
        # drives its tail): no clash, and it neither suppresses nor is suppressed.
        for tail_device in self._get_devices_for_type(bit_slot, SlotOccupancyType.TAIL):
            if tail_device == device:
                should_suppress = True
            elif is_cds:
                ...
                self.logger.warning('Bus clash detected (handover vs tail)', extra={
                    'row': row,
                    'column': column,
                    'device': device,
        ...
        # A CDS guard column may have several occupants (one per driving source);
        # when is_cds, a different source's guard is time-multiplexed with this
        # handover, so it neither clashes nor suppresses (see the tail note above).
        for guard_device in self._get_devices_for_type(bit_slot, SlotOccupancyType.GUARD):
            if guard_device == device:
                should_suppress = True
            elif is_cds:
                ...
                self.logger.warning('Bus clash detected (handover vs guard)', extra={
                    'row': row,
                    'column': column,
                    'device': device,

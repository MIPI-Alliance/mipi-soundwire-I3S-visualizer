...
if TYPE_CHECKING:
    from ..models import DataPort, Interface
    from ..models.flow_control_port import FlowControlPortConfig
...
class DataPortValidator:
    ...
    def _validate_ranges(self, result: ValidationResult,
        ...
        from ..config.constants import DataPortRanges, SpecialDevices
        from ..models import FlowMode
    ...
    def _validate_settings(self, result: ValidationResult, data_port: 'DataPort') -> None:
        ...
        if config.FlowMode_REG in (FlowMode.RX_CONTROLLED, FlowMode.ASYNC):
            ...
            self._check_fcp_fits_row(result, fcp_config, bool(config.PortDirection_REG))
        else:
    ...
    def _check_bitwidth_fits_remaining_columns(self, result: ValidationResult, config: Any) -> None:
        ...
        A coarse first-pass bound (it ignores HorizontalStart and the per-bit span);
        the PRECISE straddle guard is _check_no_wide_bit_straddles_window (C07), which
        simulates the transport UI-by-UI from HorizontalStart. Keep both — this one
        catches the gross case early with a clear message."""
    ...
    def _check_no_wide_bit_straddles_window(self, result: ValidationResult, config: Any,
        ...
        # Bits per single channel (the per-group total scales by that group's channel
        # count, which is smaller for a partial trailing group).
        per_channel_bits = (config.SampleSize_REG + 1) * (config.SampleGrouping_REG + 1)

        col = h_start
        for group_idx in range(num_groups):
            # The last group holds only the leftover channels when num_channels isn't
            # a multiple of the grouping; using the full group width would simulate
            # nonexistent bits (a spurious straddle, or masking a real one).
            group_channels = min(effective_channel_grouping,
                                 num_channels - group_idx * effective_channel_grouping)
            for bit_idx in range(per_channel_bits * group_channels):
    ...
    def _check_fcp_fits_row(self, result: ValidationResult,
                            fcp_config: 'FlowControlPortConfig', dp_is_sink: bool) -> None:
        """FCP (DRQ + optional guard + tails) shall fit in the row starting at
        FCP_HorizontalStart. The FCP runs opposite its data port: only a SINK DP's
        (source) FCP drives guard/tail — a source DP's (sink) FCP places just the
        DRQ (see FlowControlPort._arm_guard_tail, which returns early for a source
        DP). So count guard/tail only for a sink DP, or a source-DP FCP is
        over-reserved and a valid layout is falsely rejected."""
        fcp_total_width = fcp_config.FCP_BitWidth_REG + 1        # DRQ, both directions
        if dp_is_sink:
            if fcp_config.FCP_GuardEnable_REG:
                fcp_total_width += 1                             # the guard is a single UI

...
class Device:
    ...
    # ------------------------------------------------------------------
    # Bus I/O. The device drives the bus on behalf of its DataPorts and
    # FlowControlPorts. Each tick the device records the current slot's
    # state into `self._current_slot` (a BitSlotState) via its
    # `_build_dp_slot` / `_build_fcp_slot` / `_record_dp` / `_record_fcp`
    # / `_record_held` helpers; the engine reads `_current_slot` to emit
    # bus metadata. Subclasses may override to drive real hardware.
    # ------------------------------------------------------------------
        ...
        for i in range(16):
            if config.EnableCh_REG & (1 << i):
                ...
                if count == index:
                    return i
        raise IndexError(
            f"channel_index {index} exceeds enabled channel count "
            f"({bin(config.EnableCh_REG).count('1')})"
        )

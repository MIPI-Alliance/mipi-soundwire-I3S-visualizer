...
if TYPE_CHECKING:
    from ..drawing.clash_detector import ClashInfo
...
class BitInfo:
    ...
    Attributes:
        ...
        sample: Global sample number (None for non-payload slots: guard, tail,
            CDS, S0, S1, HANDOVER, DRQ, EMPTY — anywhere BitSlotState.data is None)
        bit: Bit position within the sample (None for non-payload slots;
            see `sample` for the exhaustive list)
    ...
    def row(self) -> int:
        ...
        return self.bit_index // (self._num_columns or 1)
...
@dataclass
class BusModelWarnings:
    """Warnings/diagnostics collected while building a BusModel.

    Split off from BusModel so the primary clash outputs (bus_clashes /
    device_clashes / read_overlaps) stay on the model proper, while the
    broader set of validation / mismatch / truncation warnings lives in
    one cohesive bag.
    """
...
class BusModel:
    ...
    Attributes:
        ...
        warnings: Container for all validation / mismatch / truncation warnings
    """
    num_rows: int
    num_columns: int
    ...
    # Position index: bit_index -> the bits at that index, kept in sync with `bits`
    # by add_bit / remove_bits_matching. Makes get_bits_at O(1) instead of a full
    # scan (it is called per clash and per handover candidate while building). Not
    # part of equality/repr — `bits` remains the authoritative, ordered list.
    ...
    # Warnings / diagnostics collected while building the model
    warnings: BusModelWarnings = field(default_factory=BusModelWarnings)
    ...
    def add_bit(self, bit: BitInfo) -> None:
        ...
        """
        # Bounds check: an out-of-bounds index would derive a bogus (row, column)
        # via the BitInfo properties, so skip it rather than place a phantom bit.
        if bit.bit_index < 0 or bit.bit_index >= self.total_bits:
            logger.warning(f'add_bit: bit_index {bit.bit_index} out of bounds [0, {self.total_bits}) - skipped')
            return

        # Set the num_columns for row/column derivation
        bit._num_columns = self.num_columns

        # Append to list (allow multiple bits at same position) + position index.
        self.bits.append(bit)
        self._bits_by_index.setdefault(bit.bit_index, []).append(bit)
    ...
    def get_bits_at(self, bit_index: int) -> List[BitInfo]:
        ...
        """
        return list(self._bits_by_index.get(bit_index, ()))
    ...
    def remove_bits_matching(self, bit_index: int, device: int, slot_type: SlotType) -> int:
        ...
        if not bucket:
            return 0
        matched = [b for b in bucket
                   if b.device == device and b.slot == slot_type]
        if not matched:
            return 0
...
class BusModelJSONEncoder(json.JSONEncoder):
    ...
    def _build_warnings_dict(self, model: BusModel) -> Dict[str, List[int]]:
        ...
        if model.warnings.interval_overflow_warnings:
            truncation_dps: List[int] = []
            for dp_name, _, _ in model.warnings.interval_overflow_warnings:
        ...
        if model.warnings.validation_issues:
            validation_dps: List[int] = []
            for name, _ in model.warnings.validation_issues:

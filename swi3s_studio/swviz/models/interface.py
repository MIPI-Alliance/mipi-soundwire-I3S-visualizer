...
from ..config.constants import InterfaceRanges
from ..utils.descriptors import ValidatedBool, ValidatedFloat, ValidatedInt

# DataPort / Device / FCP are referenced at runtime in __init__ and in
# type annotations. Module-level imports work because:
#   * Device imports DataPort + FlowControlPort at module load (hard).
#   * DataPort / FCP import Interface / Device only under TYPE_CHECKING.
# So interface.py is safe as an eager importer.
...
class Interface:
    ...
    def __init__(self) -> None:
        ...
        from ..config.defaults import reset_port_configs
        reset_port_configs(self)
    ...
    def reset_to_defaults(self) -> None:
        ...
        """
        self.NumColumns_REG = 15  # Register value 15 = 16 actual columns
        self.phy3_enabled = False
        self.s0_width = Interface.MIN_S0_WIDTH
        self.s1_width = Interface.MIN_S1_WIDTH
        self.cds_handover_enabled = True
        self.s1_handover_enabled = True
        self.CDS_GuardEnabled_REG = False
        ...
        # Per-source CDS guard/tail: the CDS (column 0) is time-multiplexed, so
        # the Manager and each peripheral device can drive their own guard
        # polarity / tail width without a bus clash. 13 entries: index 0 =
        # Manager, index d+1 = Device d (0-11). Guard values: 0 Off, 1 G0,
        # 2 G1. Tail values: width 0-3. CDS_GuardEnabled_REG / CDS_GuardPolarity_REG
        # (Manager's) / CDS_TailWidth_REG (max width) remain the derived
    ...
    def get_fcp(self, dp_index: int) -> 'FlowControlPort':
        ...
        Raises:
            ValueError: If dp_index is outside the valid data-port range.
        """
        if not 0 <= dp_index < self.NUM_DATA_PORTS:
            raise ValueError(
                f'get_fcp: dp_index {dp_index} out of range [0, {self.NUM_DATA_PORTS})'
            )

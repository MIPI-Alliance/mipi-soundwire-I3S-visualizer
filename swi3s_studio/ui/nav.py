...
def match_timeline_label(line, font, scale: float = 1.0) -> None:
    """Style a pyqtgraph InfiniteLine's text label (bookmark A1/A2/…) after the Timeline's
    labels. `font` is the pane's QWidget font (== the Timeline's); `scale` shrinks it (the
    Audio/Capture panes use ~0.75 so the label doesn't crowd the dense waveform). Best-
    effort across pyqtgraph versions — label internals differ, so guard everything."""
    ...
    if scale != 1.0:
        if f.pointSizeF() > 0:
            ...
            f.setPixelSize(max(1, int(round(f.pixelSize() * scale))))
    try:
        lbl.textItem.setFont(f)
    except Exception:                      # noqa: BLE001 — never break marker creation
        return
    for refresh in ("updateText", "updatePosition"):
        fn = getattr(lbl, refresh, None)
        if callable(fn):
            try:
                fn()

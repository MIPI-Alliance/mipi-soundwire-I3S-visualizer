...
class SymbolView(RowOriginMixin, QTableWidget):
    ...
    def __init__(self) -> None:
        ...
        enable_copy(self, multi=True)             # ⌘/Ctrl+C copies selected symbol rows (TSV)
    ...
    def _on_select(self) -> None:
        ...
        if rows and 0 <= rows[0].row() < len(self._symbols):
            self.sampleSelected.emit(int(self._symbols[rows[0].row()]["start_sample"]))
    ...
    def select_bus_row(self, bus_row) -> None:
        ...
        if bus_row is not None:
            idx = next((i for i, s in enumerate(self._symbols)
                        if int(s.get("row", 0)) == int(bus_row)), 0)

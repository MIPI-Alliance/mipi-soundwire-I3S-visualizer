...
class VizTheme:
    ...
    # Declared, not assigned: apply_palette() setattr()s every palette key onto this
    # class at import. Bare annotations give a type checker the names and types without
    # creating class attributes, so runtime behaviour is unchanged. Before this, mypy
    # saw NONE of them and reported 243 'type[VizTheme] has no attribute' errors across
    # 15 files — 54% of the whole tree's type debt, from one dynamic-assignment pattern.
    # tests/test_theme.py pins this list against the palettes so a new colour cannot be
    # added without declaring it.
    WINDOW_BG: str
    FRAME_BG: str
    ENTRY_BG: str
    BORDER: str
...
def authoring_stylesheet() -> str:
    ...
    QSpinBox:disabled, QDoubleSpinBox:disabled {{ color: {t.TEXT_DIM}; }}
    /* Keep the spin buttons on the same themed entry background (the default
       platform buttons render dark on the light theme). */
    QSpinBox::up-button, QSpinBox::down-button,
    QDoubleSpinBox::up-button, QDoubleSpinBox::down-button {{
        background: {t.ENTRY_BG}; border: none; width: 14px;
    }}
    /* border:none drops the frame, but then Qt no longer reserves space for the
       title and it overlaps the first row — so give the box a top margin and pin the
       title into it (subcontrol-origin:margin). */
    QGroupBox {{ border: none; margin-top: 18px; }}
...
def analyzer_stylesheet() -> str:
    ...
    /* Styling ::item above disables Qt's built-in selection fill, so the parent
       selection-background-color no longer paints — the row would select (and copy)
       but show NO highlight. Restore it explicitly, and keep it visible when the view
       is inactive (selecting drives the shared cursor, which often moves focus to
       another pane) so the selected row stays obvious. */
    ...
    QLineEdit:disabled, QSpinBox:disabled, QDoubleSpinBox:disabled {{ color: {t.TEXT_DIM}; }}
    QSpinBox::up-button, QSpinBox::down-button,
    QDoubleSpinBox::up-button, QDoubleSpinBox::down-button {{
        background: {t.ENTRY_BG}; border: none; width: 14px;
    }}
...
def chrome_stylesheet() -> str:
    ...
    /* NB: no `QDockWidget::close-button` QSS here on purpose, and the title bar is
       replaced by a custom widget (_DockTitleBar in main_window). The native style
       paints a pale button frame behind the X that a QSS `::close-button` background
       can't suppress (QStyleSheetStyle re-renders the sub-control), so the close
       button is owned + themed directly there instead. */

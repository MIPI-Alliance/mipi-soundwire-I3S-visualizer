...
# (label, color, meaning) — single source of truth for both the painter below and
# the Help ▸ Timeline Legend dialog, so the key never drifts from what's drawn.
MARK_LEGEND = [
    ("CRC error",     _C_ERROR,    "Manager-packet CRC failed (tall red tick)"),
    ("Commit + SSP",  _C_SSP,      "Confirmed commit carrying a Stream Sync Point (tall gold tick)"),
    ("Commit",        _C_COMMIT,   "Commit phase, not yet SSP-confirmed (short blue tick)"),
    ("Ping",          _C_PING,     "Ping phase (short gray tick)"),
    ("Read",          _C_READ,     "Read phase (short violet tick)"),
    ("Write / other", _C_OTHER,    "Write or other command phase (short green tick)"),
    ("Cursor",        _C_CURSOR,   "Time cursor — shared across every pane (white line)"),
    ("Bookmark",      _C_BOOKMARK, "Bookmarked sample (magenta diamond at the baseline)"),
    ("Link bring-up", _C_BRINGUP,  "§5.1.2 Bus Reset → PHY-select before audio mode "
...
_TICK_RANK = {
    ...
    "Commit": 2, "Commit + SSP": 3, "CRC error": 3,
}
...
class TimelineRibbon(RowOriginMixin, QWidget):
    ...
    def retheme(self) -> None:
        ...
        global _C_CURSOR, _C_AXIS, _C_BOOKMARK, _C_BRINGUP, MARK_LEGEND
        _C_ERROR = QColor(VizTheme.SEM_ERROR)
        _C_SSP = QColor(VizTheme.SEM_SSP)
        _C_COMMIT = QColor(VizTheme.SEM_COMMIT)
        _C_PING = QColor(VizTheme.SEM_PING)
    ...
    def set_segments(self, segments, total_samples: int = 0) -> None:
        ...
        distinct bus geometry shows in its own colour."""
    ...
    def set_bookmarks(self, samples) -> None:
        self._bookmarks = list(samples)
        self.update()
    ...
    def set_nav_starts(self, starts) -> None:
        """Restrict Left/Right arrow navigation to these (sorted) command start samples —
        the command table's currently-VISIBLE commands, so arrow nav respects the active
        filter. None (the default) navigates every command."""
    ...
    def paintEvent(self, _event) -> None:
        ...
        tr = self._track_rect()
        # Bus-config bands (behind everything): one translucent fill per segment,
        # coloured by column count, labelled "{cols}col" where there's room. Use a
        # compact font in a full-height label rect so the text isn't clipped, and
        # nudge it right of the segment edge so a bring-up flag can't cover it.
        ...
        for i, seg in enumerate(self._segments):
            ...
            p.fillRect(QRectF(x0, tr.top(), max(1.0, x1 - x0), tr.height()),
                       self._seg_band.get(cols, _CONFIG_BANDS[0]))
            if x1 - x0 > 30:
                p.setFont(label_font)
                p.setPen(QPen(QColor(VizTheme.TEXT)))     # theme text — visible in light AND dark
                p.drawText(QRectF(x0 + 6, tr.top(), x1 - x0 - 8, 16),
                           Qt.AlignLeft | Qt.AlignVCenter, self._segment_label(i, cols))
        ...
        if self._bringup:
            ...
            # LC_Request), each its own data-port-palette colour, with a label where the
            # band is wide enough (visible once zoomed into the bring-up region).
            ...
            for sec in self._link_sections:
                ...
                w = max(2.0, sx1 - sx0)
                sband = QRectF(sx0, tr.top(), w, tr.height())
                ...
                if w > 44:
                    ...
                    p.drawText(QRectF(sx0 + 3, tr.top(), w - 5, 14),
            ...
            if not self._link_sections:                       # fallback: single cyan band
                ...
                band = QRectF(bx0, tr.top(), max(4.0, bx1 - bx0), tr.height())
        ...
        # event ticks. A long capture is ~98% Pings; with ~7M samples per pixel
        # many commands collide on one x. Draw by ASCENDING importance so the
        # significant marks WIN the pixel (Ping < other < commit < SSP/error),
        # instead of last-in-time-order overpainting a Write with a later Ping.
        # (Order is precomputed in set_events — not re-sorted per frame.)
        for cmd in self._draw_order:
            ...
            top = tr.top() if tall else tr.top() + tr.height() * 0.35
            p.drawLine(int(x), int(top), int(x), int(tr.bottom()))
        # Commit points: where a confirmed sync-point commit actually TAKES EFFECT (the
        # SSP, Row_Delay rows after the command). Same commit colour as the command tick
        # but DOTTED, so the deferred effect is visible and distinct from the command.
        if self._commit_points:
            dot = QPen(_C_COMMIT, 1, Qt.DotLine)
            p.setPen(dot)
            for cp in self._commit_points:
                if self._in_view(cp):
                    cx = self._x_for(cp)
                    p.drawLine(int(cx), int(tr.top()), int(cx), int(tr.bottom()))
        # cursor
        ...
        # bookmarks (magenta diamonds at the baseline)
        p.setPen(QPen(_C_BOOKMARK, 1))
        p.setBrush(_C_BOOKMARK)
    ...
    def _segment_label(self, index: int, cols: int) -> str:
        """Config-band label. The initial audio segment of a cold/warm start is the
        PHY's Safe-Lock geometry, so label it by the sequence + selected PHY (e.g.
        'Cold Start PHY2') instead of the bare '{cols}col' — the PHY is what makes
        that 2-column (FBCSE) / 4-column (DLV) region meaningful."""
        b = self._bringup
        if (index == 0 and b and b.get("phy_name")
                and b.get("safe_lock_columns") == cols):
            seq = str(b.get("sequence", "")).title()      # 'Cold' / 'Warm'
            return f"{seq} Start {b['phy_name']}"
        return f"{cols}col"
    ...
    def _hover_text(self, x: float) -> str:
        ...
        if cmd is not None:
            ...
            meaning = next((m for n, _c, m in MARK_LEGEND if n == label), "")
            s = int(cmd.get("start_sample", 0))
            extra = f" — {meaning}" if meaning else ""
            return (f"{cmd.get('command', '')} ({label}){extra} · "
                    f"row {self.display_row(cmd.get('bus_row', 0)):,} · {when(s)}")
        sample = self._sample_for(x)
        cp = self._nearest_commit_point(x)
        if cp is not None:
            return f"Commit Point (SSP) · {when(cp)}"
        ...
        if seg is not None:
            i, s = seg
            return (f"Bus config: {self._segment_label(i, int(s.get('column_count', 0)))} "
                    f"· from row {int(s.get('row_base', 0)):,}")
        return ""

    def _nearest_commit_point(self, x: float, px: float = 5.0):
        """The commit-point (SSP) effective sample within `px` of x, or None."""
        best, best_d = None, px
        for cp in self._commit_points:
    ...
    def mouseMoveEvent(self, event) -> None:
        ...
        if event.buttons() & Qt.LeftButton:
            self.seeked.emit(self._sample_for(x))
            return

...
A PHY selector (PHY1 / PHY2) picks the specification. PHY1 (FBCSE-slow) and PHY2
(FBCSE-fast) share the setup/hold inequality structure exactly — the SWI3S spec
documents them together (§11.2) and points both at the same RX/TX timing tables
and measurement waveforms — so only the Specification values differ (PHY1's
Peripheral/Manager high-Z and output-hold times are slower; its mandatory F_CLK is
half). The Specification column is read-only; only the Example column is editable.
...
- A top control bar: PHY selector, F_CLK target, Reset, and Save / Load of the
  Example column (YAML, or JSON if PyYAML is absent).
...
# and the SAME setup/hold measurement waveforms (Figures 174–179). Only the output
# timing values in Table 129 (and the mandatory F_CLK in Table 126) differ: PHY1's
# Peripheral output-hold / high-Z-recovery and Manager high-Z-recovery are much
# slower. Everything else (V thresholds, Man_t_DD, t_IS/t_IH, t_DZ, t_RF, noise,
# bus) is a shared "PHY1 & PHY2" value, so we override only what the spec splits.
_PHY1_SPEC: Dict[str, ParamRange] = {
    # Table 129 §11.2.3 — PHY1 columns (PHY2 values, for contrast, in default rows).
...
# gives no nominal; the target defaults match the SE reference spreadsheet.
_PHY_FCLK_TARGET: Dict[int, float] = {1: 6.4, 2: 13.2}
...
class TimingView(QWidget):
    def __init__(self) -> None:
        super().__init__()
        self._loading = False
        self._phy = 2                                     # selected PHY (1 or 2)
        ...
        self._title = QLabel("SWI3S PHY2 Timing Calculator")
    ...
    def _build_top_bar(self) -> QVBoxLayout:
        # Two rows so nothing clips when the bar is pinned to the (narrower) results
        # width: row 1 is the PHY picker + Reset/Save/Load; the Clock-Frequency Target
        # sits on row 2 beneath them.
        outer = QVBoxLayout()
        ...
        # PHY selector — PHY1 (FBCSE-slow) and PHY2 (FBCSE-fast) share the inequality
        # structure; picking one reseeds the read-only Spec column from that PHY's
        # tables and sets the mandatory-max-derived F_CLK target.
        bar.addWidget(QLabel("PHY"))
        self._phy_combo = QComboBox()
        self._phy_combo.addItem("PHY1", 1)
        self._phy_combo.addItem("PHY2", 2)
        self._phy_combo.setMinimumWidth(_px(80))        # fit "PHY1"/"PHY2" + the dropdown arrow
        self._phy_combo.setCurrentIndex(1)              # default PHY2 (before connect)
        self._phy_combo.currentIndexChanged.connect(self._on_phy_changed)
        bar.addWidget(self._phy_combo)
        ...
        # Row 2 — Clock Frequency Target, beneath the PHY picker / buttons.
        clk = QHBoxLayout()
    ...
    def set_from_dict(self, d: dict) -> None:
        ...
        try:
            ...
            # falls back into the now-shared picker. A legacy dict with no "phy" key
            # predates the PHY selector → PHY2 (the only PHY it could have held).
            phy = int(d.get("phy", 2))
            i = self._phy_combo.findData(phy)
            if i >= 0:
                self._phy_combo.setCurrentIndex(i)       # updates _phy + title, no reseed
            if "fclk_target" in d:
    ...
    def _update_title(self) -> None:
        self._title.setText(f"SWI3S PHY{self._phy} Timing Calculator")

    def _on_phy_changed(self, *_a) -> None:
        """Switching PHY reseeds the Spec column (and the Example starting point) from
        that PHY's spec tables and resets the F_CLK target. During a workspace load
        (`_loading`) we only track the selection — the saved values are applied after."""
        self._phy = int(self._phy_combo.currentData())
        self._update_title()
    ...
    def _reset_to_defaults(self) -> None:
        self._loading = True
        try:
            self._fclk.setValue(_PHY_FCLK_TARGET.get(self._phy, 13.2))
            self._set_shape(self._man_shape, "linear")
            self._set_shape(self._per_shape, "linear")
            for side in SIDES:
                for row in _ALL_ROWS:
                    rng = _spec_range(row.name, self._phy)

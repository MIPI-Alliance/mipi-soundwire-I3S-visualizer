...
from PySide6.QtWidgets import (QComboBox, QHBoxLayout, QLabel, QMenu, QPushButton,
...
class EyeView(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self._timing = None
        self._capture = None
        self._sections = []           # per-section UI stats (kept for callers; not drawn)
        self._segments = []           # decode segments (start_ui, column_count) for tr_column
        self._regions = []            # audio-mode regions (Session.timing_regions)
        self._roles_fn = None         # Session.timing_column_roles — column roles on demand
        self._roles_cache = {}        # region index -> column-role map (lazy, cached)
        self._col_roles = {}          # column -> driver role (from the current region)
        self._start_sample = 0        # current region's sample range (excludes link control)
        self._end_sample = None
        self._filter_items = []       # [{action, cols(frozenset), kind}] checkable filter items
        self._dirty = False
        self._cat_hidden = set()      # hidden edge flavours: {(clk_rising, data_rising)} keys
        self._worst = []              # tightest transitions (sample, setup_ns, hold_ns)
        self._worst_i = 0

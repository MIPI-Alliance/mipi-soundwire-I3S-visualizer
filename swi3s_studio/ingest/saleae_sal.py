...
def available_memory_bytes() -> int:
    ...
    machine, i.e. failing OPEN on exactly the machines that can least afford it). A hand-built
    venv on a test guest is what exposed it; `run.ps1` and `run.sh` install requirements.txt,
    so a user following the README would not have hit it.
    """
...
def memory_budget(max_bytes: Optional[int] = None) -> int:
    ...
    (`budget_source`). Free RAM, then a quarter of TOTAL RAM, then a small constant — each
    weaker than the last and each therefore smaller, because the less is known the less
    should be risked. It used to fall straight to a flat 8 GiB, which was the same number as
    the cap and so invisible on a large machine while being ~3.6x too permissive on a 4 GB
    one: failing OPEN on precisely the machines that can least afford it, in the branch whose
    own comment promised to fail closed.
    """
